...
def train():
    ...
    train_transform = torchvision.transforms.Compose([torchvision.transforms.Resize(resize),
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            model_file = os.path.join(save_model_path, 'resnet34_CIFAR_10_train_fc_epoch_%d_acc_%g.pth' %
                                      (best_epoch, best_acc))
            if os.path.isfile(model_file):
                os.remove(os.path.join(save_model_path, 'resnet34_CIFAR_10_train_fc_epoch_%d_acc_%g.pth' %
                                       (best_epoch, best_acc)))
            end_patient = 0
            best_acc = test_acc
            best_epoch = epoch + 1
            ...
            torch.save(model.state_dict(), os.path.join(save_model_path,
                                                        'resnet34_CIFAR_10_train_fc_epoch_%d_acc_%g.pth' %
                                                        (best_epoch, best_acc)))
        else:
            end_patient += 1
...
def get_statistic():
    ...
    train_data = datasets.ImageFolder('D://Code//BilinearCNN//data//train',
